import { toastError } from "@/components/custom/toast";
import { sendPost } from "@/utils/sendRequest";
import { Spinner } from "@chakra-ui/react";
import FacebookLogin from "@greatsumini/react-facebook-login";
import { useState } from "react";
import { useRouter } from 'next/navigation';

const LoginWithFacebookButton = ({onClick,setLoading}) => {
    const handleClick = ()=>{
        setLoading(1);
        onClick();
    }
    return <div>
        <button className="gsi-material-button" style={{width:'100%'}} onClick={handleClick}>
            <div className="gsi-material-button-state" />
            <div className="gsi-material-button-content-wrapper">
                <div className="gsi-material-button-icon">
                    <svg xmlns="http://www.w3.org/2000/svg" shapeRendering="geometricPrecision" textRendering="geometricPrecision" imageRendering="optimizeQuality" fillRule="evenodd" clipRule="evenodd" viewBox="0 0 515 515">
                        <g fillRule="nonzero">
                            <path fill="#0866FF" d="M509 254.5C509 113.94 395.06 0 254.5 0S0 113.94 0 254.5C0 373.86 82.17 474 193.02 501.51V332.27h-52.48V254.5h52.48v-33.51c0-86.63 39.2-126.78 124.24-126.78 16.13 0 43.95 3.17 55.33 6.33v70.5c-6.01-.63-16.44-.95-29.4-.95-41.73 0-57.86 15.81-57.86 56.91v27.5h83.13l-14.28 77.77h-68.85v174.87C411.35 491.92 509 384.62 509 254.5z"/>
                            <path fill="#fff" d="M354.18 332.27l14.28-77.77h-83.13V227c0-41.1 16.13-56.91 57.86-56.91 12.96 0 23.39.32 29.4.95v-70.5c-11.38-3.16-39.2-6.33-55.33-6.33-85.04 0-124.24 40.16-124.24 126.78v33.51h-52.48v77.77h52.48v169.24c19.69 4.88 40.28 7.49 61.48 7.49 10.44 0 20.72-.64 30.83-1.86V332.27h68.85z"/>
                        </g>
                    </svg>
                </div>
                <span className="gsi-material-button-contents">Continue with facebook</span>
                <span style={{ display: 'none' }}>Continue with facebook</span>
            </div>
        </button>
    </div>
}


const LoginWithFacebook = () => {

    const [loading,setLoading] = useState(0);
    const router = useRouter()

    const handleSuccess = (res)=>{
        
    }

    
    const handleError = (e)=>{

        console.log(e)
        toastError("Facebook login failed.")
        setLoading(0)
    }
    const handleProfileSuccess = async (response) => {

        const registerResponse = await sendPost({url:`api/user`,payload:{authType:'facebook',...response}});
        if(!registerResponse?.success){
            toastError(registerResponse?.message || 'Something went wrong');
            setLoading(0);
            return;
        }
        router.push('/dashboard')
        setLoading(0)
    }
    
    return (
        <>
            {
                loading ==0 ?
                    <FacebookLogin
                        appId={process.env.NEXT_PUBLIC_FACEBOOK_APP_ID}
                        onSuccess={handleSuccess}
                        onFail={handleError}
                        onProfileSuccess={handleProfileSuccess}
                        scope="public_profile,email"
                        
                        render={(renderProps) => <LoginWithFacebookButton {...renderProps} setLoading={setLoading}/>}
                    />:<div style={{display:'flex',justifyContent:'center'}}>
                                        <Spinner size={'lg'}/>
                                    </div>
            }
            
        </>
    );
}

export default LoginWithFacebook